...
walkaddr(pagetable_t pagetable, uint64 va)
{
  ...
  //konverterer til fysisk adresse
  pa = PTE2PA(*pte);
  return pa;
}
...
{
  uint64 i;
  //peker til en page table entry
  pte_t *pte_old;

  //itererer gjennom minneområdet
  for(i = 0; i < sz; i += PGSIZE){

    //sjekker pte
    if((pte_old = walk(old, i, 0)) == 0){
      panic("uvmcopy: pte should exist here");
      return -1;
    }
    
    //sjekker om pte er gyldig
    if((*pte_old & PTE_V) == 0){
      panic("uvmcopy: the page is not present");
      return -1;
    }
    
    //sjekker om adressen er sidejustert
    if (i % PGSIZE != 0) {
        panic("uvmcopy: the virtual address is not pagealigned");
        return -1;
      }
    ...
    // fjerner parents page table mapping
    uvmunmap(old, i, 1, 0);
