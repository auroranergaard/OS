...
int fork(void)
{
    int i, pid;
    struct proc *np;
    struct proc *p = myproc(); //peker til prosessen som kaller fork


    // allokerer en ny prosess
    if ((np = allocproc()) == 0)
    {
        return -1;
    }

    // kopierer minnet fra forelder p til barneprosesen np
    // om kopiering feiler, frigjør ressursene 
    ...
    // kopier registerverdier og sett returverdi for barneprosessen
    *(np->trapframe) = *(p->trapframe);

    // Cause fork to return 0 in the child.
    np->trapframe->a0 = 0;

    // kopierer filer og arbeidsmappe fra forelder til barn
    for (i = 0; i < NOFILE; i++)
        if (p->ofile[i])
            np->ofile[i] = filedup(p->ofile[i]);
    np->cwd = idup(p->cwd);

    // kopierer navnet på prosessen
    safestrcpy(np->name, p->name, sizeof(p->name));

    //pid tildeles barneprosessens pid
    pid = np->pid;
